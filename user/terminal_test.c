...
void test_terminal_io(void) {
  TracePrintf(0, "=== Testing Terminal I/O ===\n");
  
  // Test TtyWrite
  char *message = "Hello from Yalnix! Type something and press Enter:\n";
  int tty_id = 0;  
  
  TracePrintf(0, "Writing to terminal %d\n", tty_id);
  ...
  TracePrintf(0, "Wrote %d bytes to terminal %d\n", bytes_written, tty_id);
  
  // Test TtyRead
  char buffer[100] ;
  TracePrintf(0, "Reading from terminal %d\n", tty_id);
  ...
  TracePrintf(0, "Read %d bytes from terminal %d: '%s'\n", 
